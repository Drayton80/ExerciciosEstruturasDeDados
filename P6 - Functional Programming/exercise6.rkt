...
;; --- EXERCISE VI --------------------------------------------------------------------
;; Padrões de recursividade e funções de alta ordem - Parte 2
;;
;; --- Questão 1 ----------------------------------------------------------------------
...
(define (filter p lst)
  (define (tail-filter p lst aux-lst)
    (if (empty? lst)
      aux-lst
      (if (p (first lst))
        (tail-filter p (rest lst) (cons (first lst) aux-lst))
        (tail-filter p (rest lst) aux-lst)
      )
    )
  )

  (reverse (tail-filter p lst '()))
)
...
(define (filtra-par-lg lst)
  (define (filtra-integer lst aux-lst)
    (if (empty? lst)
      (reverse aux-lst)
      (if (integer? (first lst))
        (filtra-integer (rest lst) (cons (first lst) aux-lst))
        (filtra-integer (rest lst) aux-lst)
      )
    )
  )

  (filter even? (filtra-integer lst '()))
)
...
(define (tamanho-lista-strings lstr)
  ...
  (fold-right + 0 (map string-length lstr))
)
...
(define (todos? p lst)
  (if (empty? lst)
    #t
    (and (p (first lst)) (todos? p (rest lst)))
  )
)
...
(define (algum? p lst)
  (if (empty? lst)
    #f
    (or (p (first lst)) (algum? p (rest lst)))
  )
)
...
(define (partition p lst)
  (define (tail-partition p lst p-lst not-p-lst)
    (if (empty? lst)
      (list (reverse p-lst) (reverse not-p-lst))
      (if (p (first lst))
        (tail-partition p (rest lst) (cons (first lst) p-lst)                   not-p-lst )
        (tail-partition p (rest lst)                   p-lst  (cons (first lst) not-p-lst))
      )
    )
  )

  (tail-partition p lst '() '())
)
...
(define (menor-quociente x lst)
  ...
  (define (min-lista l [min (first l)])
    (if (empty? l)
      min
      (if (< (first l) min)
        (min-lista (rest l) (first l))
        (min-lista (rest l)  min     )
      )
    )
  )

  (min-lista (map (lambda (y) (if (= y 0)
                                  0 (/ x y))) lst))  
)

(define-test-suite test-menor-quociente
  (test-equal? "inteiros" (menor-quociente 12 (list 2 3 4)) 3)
  (test-equal? "possui zero" (menor-quociente 12 (list 2 0 4)) 0)
)
